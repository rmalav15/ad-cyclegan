...
def concat_gaussian_noise(image, len=32, stdv=0.2):
    with tf.name_scope('Add_gaussian_noise'):
        image_shape = tf.shape(image)
        noise_shape = (image_shape[0], image_shape[1], image_shape[2], len)
        noise = tf.random_normal(shape=noise_shape, mean=0.0, stddev=stdv, dtype=tf.float32)
        noise_img = tf.concat([image, noise], axis=-1)
    return noise_img
...
def generator_unet(image, options, reuse=False, name="generator", a2b=True):
    ...
    with tf.variable_scope(name):
        ...
        e8 = concat_gaussian_noise(e8, stdv=options.noise_stdv)
